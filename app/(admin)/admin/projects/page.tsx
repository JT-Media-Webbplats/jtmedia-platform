    ...
    <div className="p-8">
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="text-white/30 text-sm mt-1">{activeCount} aktiva · {completedCount} avslutade</p>
        </div>
      ...
      <div className="bg-white/4 border border-white/6 rounded-2xl overflow-hidden">
        {!projects || projects.length === 0 ? (
          <div className="px-6 py-16 text-center">
            <p className="text-white/30 text-sm">Inga projekt ännu.</p>
          </div>
        ) : (
            ...
            <thead>
              <tr className="border-b border-white/6">
                {['Projekt', 'Kund', 'Status', 'Timmar', 'Uppdaterad'].map((h) => (
                  <th key={h} className="text-left px-6 py-4 text-xs font-semibold uppercase tracking-widest text-white/25">{h}</th>
                ))}
              </tr>
            </thead>
                ...
                return (
                  <tr key={p.id} className="hover:bg-white/4 transition-colors">
                    <td className="px-6 py-4">
                      <Link href={`/admin/projects/${p.id}`} className="font-semibold text-white hover:text-brand-green transition-colors">
                        {p.name}
                      </Link>
                    </td>
                    <td className="px-6 py-4">
                      {customer ? (
                        <Link href={`/admin/customers/${customer.id}`} className="text-white/50 hover:text-white/80 transition-colors">
                          {customer.name}
                        </Link>
                      ) : '—'}
                    </td>
                    ...
                    <td className="px-6 py-4 min-w-[160px]">
                      {pct !== null ? (
                        <div className="flex items-center gap-2.5">
                          ...
                          <span className="text-xs text-white/30 shrink-0">{logged}/{budget}h</span>
                        </div>
                      ) : (
                        <span className="text-white/30 text-xs">{logged}h loggat</span>
                      )}
                    </td>
